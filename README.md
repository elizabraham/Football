## Completed Projects
 ## 1. NWSL player data scraping and cleaning

 Code scraping done using requests in python (json file)
 ## Code Script
 [NWSL Webscraping](https://github.com/elizabraham/Football/blob/main/Code/NWSL%20Web%20scraping%20and%20cleaning.ipynb)
 
 ## 2. Football Post Match Analysis Dashboard

 Built using Streamlit and mplsoccer to analyze match-level data.

 Key Features:
 - Match Summary
 - Shot Maps
 - Team performance

 Tech Stack:
 Python, Pandas, Streamlit, mplsoccer, Matplotlib

 ## Live Demo
 🔗[Post Match Analysis](https://elizabraham-football-codepost-match-analysis-report-dev-zarrdn.streamlit.app/)
  -Note: Upload the sample match file [.csv file](https://github.com/elizabraham/Football/tree/main/Match%20files/TAL) in  the app to view full stats.

 ## App Preview

 [Sample 1](https://github.com/elizabraham/Football/blob/main/Images/Sample%201.png)
 [Sample 2](https://github.com/elizabraham/Football/blob/main/Images/Sample%202.png)
 [Shot Map](https://github.com/elizabraham/Football/blob/main/Images/Shot%20Map.png)
 [Summary](https://github.com/elizabraham/Football/blob/main/Images/Summary.png)

 ## Code Script
 [Post Match Analysis](https://github.com/elizabraham/Football/blob/main/Code/post_match_analysis_report_dev.py)

 

## Work In Progress Projects

 ## 1. Women's Euros 2025 comparison for Ballon dor nominees 

 Built using Statsbomb and pandas to analyze player-level data.

 ## Code Script
 [Women's Euros 2025](https://github.com/elizabraham/Football/blob/main/Code/Womens_Euros_2025.py)

 ## 2. NWSL Attacking Defender Scout Analysis

 Using Context analysis to scout top attacking defenders in NWSL

 Key Features:
 - Web Scraping
 - Data Cleaning
 - Dfender Analysis, scores based on movement, progression and finish

 Tech Stack:
 Python, Pandas, requests, mplsoccer, Matplotlib

 ## Code Script
 [NWSL](https://github.com/elizabraham/Football/blob/main/Code/Test.ipynb)


